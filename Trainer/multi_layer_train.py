...
from Models.multi_layer_nn import MultiLayerClassifier
...
def train_epoch():
    ...
    for step, (x, y) in enumerate(train_loader):
        ...
        leaf_grad = grad(loss, h, create_graph=True,
                         retain_graph=True)[0]
        # Derivative of the loss wrt output i.e DL/dh

        optimizer_output.step(x_hidden, leaf_grad)
...
if __name__ == '__main__':
    ...
    model = MultiLayerClassifier(input_features=28*28).to(device=device)

    LR_HEB = 40
    LR_ADAM = 0.001
    NUM_EPOCHS = 100

    optimizer_hidden = torch.optim.Adam(model.hidden.parameters(), lr=LR_ADAM)
    optimizer_output = HebbianDescent(model.output.parameters(), lr=LR_HEB)
